...
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-4o-mini").strip()  # você pode trocar no env
...
def calcular_oee(atraso: int, meta_acumulada: int, aprovacao_perc: float) -> float:
    # Performance = aderência ao esperado (meta acumulada vs produzido)
...
# ==============================
def gerar_insights_regras(res_dia: list[dict], res_mes: list[dict]) -> list[str]:
    msgs = []

    # Performance = atraso/meta, Qualidade = aprovação + reprovados
    for r in res_dia:
        if r.get("atraso", 0) > 0:
            msgs.append(f"• **{r['nome']}**: atraso de **{r['atraso']}** (performance abaixo do esperado).")

        if r.get("inspecionado", 0) >= 10 and r.get("aprovacao", 100) < 95:
            msgs.append(
                f"• **{r['nome']}**: qualidade em **{r['aprovacao']:.1f}%** com **{r['reprovados']}** reprovados."
            )

    # Comparação dia x mês (tendência)
    for rd in res_dia:
        rm = next((x for x in res_mes if x["key"] == rd["key"]), None)
        if not rm:
            continue
        if rm.get("inspecionado", 0) >= 30 and rd.get("inspecionado", 0) >= 10:
            if rd["aprovacao"] + 2 < rm["aprovacao"]:
                msgs.append(f"• **{rd['nome']}**: aprovação do dia abaixo do mês (**{rd['aprovacao']:.1f}%** vs **{rm['aprovacao']:.1f}%**).")
            if rd["oee"] + 2 < rm["oee"]:
...
# ==============================
def _formatar_resumos_para_prompt(resumos: list[dict]) -> str:
    linhas = []
    for r in resumos:
        linhas.append(
            f"- {r['nome']}: total={r['total']}, aprovacao={r['aprovacao']:.1f}%, "
            f"inspecionado={r['inspecionado']}, reprovados={r['reprovados']}, "
            f"atraso={r['atraso']}, oee={r['oee']:.1f}%"
        )
    return "\n".join(linhas)

def gerar_insights_ia(res_dia: list[dict], res_mes: list[dict]) -> str:
    """
    Requer OPENAI_API_KEY. Retorna texto com recomendações.
    """
    if not OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY não configurada no ambiente.")

    # Import local para não quebrar se não tiver lib instalada
    from openai import OpenAI

    client = OpenAI(api_key=OPENAI_API_KEY)

    prompt = f"""
Você é um analista de produção e qualidade (Lean/Manufatura).
Performance significa "aderência ao esperado" (meta x realizado) e é representada por atraso e OEE.
Reprovação é falha de qualidade (não conforme / reprovado).

Quero insights curtos e práticos para o gestor, em português do Brasil.

DADOS DO DIA (turno/dia selecionado):
{_formatar_resumos_para_prompt(res_dia)}

DADOS DO MÊS ATUAL (até hoje):
{_formatar_resumos_para_prompt(res_mes)}

Entregue:
1) 4 a 7 bullets de INSIGHTS com foco em ações (o que investigar/ajustar).
2) Separe em "Performance" e "Qualidade".
3) Se houver reprovados, sugira possíveis frentes (ex: componente, posto, operador, reinspeção, verificação final) sem inventar fatos.
4) Evite termos técnicos demais e seja objetivo.
""".strip()

    # usa Responses API (SDK atual costuma suportar)
...
def main():
    ...
    # Sempre calcula o "dia selecionado" (para comparar nos insights)
    # - Se estiver em mês, o "dia" base vira HOJE para comparação
    dia_ref_inicio = hoje
    dia_ref_fim = hoje
    if modo == "Turno (Dia)":
        dia_ref_inicio = data_inicio
        dia_ref_fim = data_fim

    resumos_dia = [
        resumo_total_apontamentos(dia_ref_inicio, dia_ref_fim),
        resumo_mola(dia_ref_inicio, dia_ref_fim),
        resumo_manga_pnm(dia_ref_inicio, dia_ref_fim),
    ]
    ...
    if pagina == "Dashboard":
        ...
        for m in gerar_insights_regras(resumos_dia, resumos_mes):
            st.markdown(m)
        ...
        with cols[1]:
            st.caption(
                "Dica: configure `OPENAI_API_KEY` (e opcional `OPENAI_MODEL`) para habilitar."
            )

        if "insight_ia_texto" not in st.session_state:
            st.session_state.insight_ia_texto = ""

        if gerar:
            if not OPENAI_API_KEY:
                st.warning("OPENAI_API_KEY não configurada. Configure no ambiente/Secrets para liberar a IA.")
            else:
                with st.spinner("Gerando insights com IA..."):
                    try:
                        texto = gerar_insights_ia(resumos_dia, resumos_mes)
                        st.session_state.insight_ia_texto = texto
                    except Exception as e:
                        st.error(f"Falha ao gerar insights com IA: {e}")

        if st.session_state.insight_ia_texto:
            st.text_area(
                "Resultado IA",
                value=st.session_state.insight_ia_texto,
                height=220
            )
        else:
            st.info("Clique em **Gerar insights (IA)** para receber um resumo acionável.")
